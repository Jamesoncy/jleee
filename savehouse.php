...
//Input Validations
	if($model == '') {
	 ...
	 if (!preg_match("/^[a-zA-Z ]*$/",$model)){
       $errmsg_arr[] = 'Only letters and white space allowed in House Model';
		$errflag = true;
     }
	...
	if (!is_numeric($floorarea)) {
       $errmsg_arr[] = 'Only number allowed in Floor Area';
		$errflag = true;
     }
	if($lotarea == '') {
	...
	if (!is_numeric($lotarea)) {
       $errmsg_arr[] = 'Only number allowed in Lot Area';
		$errflag = true;
     }
	if($price == '') {
	...
	if (!is_numeric($price)) {
       $errmsg_arr[] = 'Only number allowed in House Price';
		$errflag = true;
     }
