...
<div class="warper container-fluid">
  ...
  <div class="page-header"><h1>Amortization <small>Monitoring</small></h1></div>
...
<input type="hidden" name="reserveid" value="<?php echo $user['reserveid']; ?>" />
		  <input type="hidden" name="subname" value="<?php echo $user['subname']; ?>" />
	<input type="hidden" name="phase" value="<?php echo $user['phase']; ?>" />
	<input type="hidden" name="block" value="<?php echo $user['block']; ?>" />
	<input type="hidden" name="lotno" value="<?php echo $user['lotno']; ?>" />
	<input type="hidden" name="buyer" value="<?php echo $user['buyer']; ?>" />
	<input type="hidden" name="email" value="<?php echo $user['email']; ?>" />
	<input type="hidden" name="terms" value="<?php echo $user['terms']; ?>"/>
	<input type="hidden" name="down" value="<?php echo $user['down']; ?>"/>
	<input type="hidden" name="amort" value="<?php echo $user['total_amort']; ?>"/>
	<input type="hidden" name="date" value="<?php echo $user['start_date'];?>"/>
		<input type="submit" name="Confirm" value="Update" onclick="return confirm('Are you sure you want to Confirm Reservation no.<?php echo $reserveid ?>');"/>
		</form> </td> <tr> 
		<?php
...
?>
  ...
  <div class="page-header"><h1>Payment <small>History</small></h1></div>
...
require_once("./dbconn.php");
				...
				$stmt1 = $conn->prepare('SELECT * from payment_history order by pay_date DESC');
				$stmt1->execute();
				while($rs1 = $stmt1->fetch()){

				$stmt = $conn->prepare('select * from reserve WHERE reserveid = ?');
				$stmt->execute(array($rs1['reserveid']));
			...
			<div class="panel panel-default">
			  ...
			  <thead>
                                  <tr>
                                    <th style="width:20%">Name</th>
                                    <th style="width:20%">Total Amortization</th>
			...
			<td>
				...
				<td>
					<?php echo $rs['total_amort']; ?>
				</td>
				<td>
					<?php echo $rs1['pay_date']; ?>
				</td>
			</tr>
			<?php
				}}?>
  ...
  <div class="page-header"><h1>Monthly and Annual <small>Sales</small></h1></div>
		...
		<h1>Monthly</h1>
			...
			<?php
				$stmt = $conn->prepare('select sum(amount) as xamount,month(pay_date) as xmonth,Year(pay_date) as xyear from payment_history Group By YEAR(pay_date),month(pay_date)');
				$stmt->execute(array($rs1['reserveid']));
			...
			<td>
				...
				<td>
					<?php echo number_format($rs['xamount'],2,'.',','); ?>
				</td>
			</tr>
			<?php
				}?>
	 ...
	 <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="basic-datatable">
			...
			<?php
				$stmt = $conn->prepare('select sum(amount) as xamount,month(pay_date) as xmonth,Year(pay_date) as xyear from payment_history Group By YEAR(pay_date)');
				$stmt->execute(array($rs1['reserveid']));
